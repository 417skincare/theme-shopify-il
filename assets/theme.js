...
doPredictiveSearch_fn = async function() {
  ...
  const isHebrew = window.Shopify.locale === 'he'; 

  const usePredictiveApi = this.supportsPredictiveApi() && !isHebrew;
  const url = `${root}search${usePredictiveApi ? "/suggest" : ""}`;
  
  const queryParams = `q=${encodeURIComponent(__privateGet(this, _queryInput).value)}&section_id=predictive-search&resources[limit]=10&resources[limit_scope]=each`;

  __privateGet(this, _fetchAbortController)?.abort();
  __privateSet(this, _fetchAbortController, new AbortController());

  try {
    const response = await cachedFetch(`${url}?${queryParams}`, { 
    ...
    const newSection = tempDoc.querySelector(".shopify-section");
    const resultsSlot = this.querySelector('[slot="results"]');

    if (newSection && resultsSlot) {
      const importedNode = document.importNode(newSection, true);
      resultsSlot.replaceChildren(...importedNode.children);
    } else {
      console.warn("Predictive Search: .shopify-section not found in response.");
...
//Item Count on Cart Icon
class CartCountBadge extends HTMLElement {
  constructor() {
    super();
    this.updateCount();
  }
  ...
  connectedCallback() {
    ...
    document.addEventListener('cart:change', () => this.updateCount());
    
    // Also listen to Shopify's native cart API events
    document.addEventListener('shopify:section:load', () => this.updateCount());
  }
...
customElements.define('cart-count-badge', CartCountBadge);
//End of item Count on Cart Icon
